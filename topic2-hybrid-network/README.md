...
### 問題2: BGP設定の残存によりトンネル接続が不安定

**原因**: BGP動的ルーティングから静的ルーティングに変更後、
以前のFRR(BGP)設定がメモリにキャッシュされたまま残っていた。
...
### 問題3: ターミナルでESTABLISHEDなのにAWSコンソールでDOWN
...
正常です。Site-to-Site VPNは自動的に2本のトンネルを作成します。

| トンネル | 役割 |
|---|---|
| Tunnel 1 | メイン接続 (現在使用中) |
| Tunnel 2 | 冗長化バックアップ (Tunnel1が生きている間はDOWN) |

Tunnel 1が障害を起こした場合、自動的にTunnel 2に切り替わります。
